...
export default function CustomQuizSectionPage() {
  const router = useRouter()
  const [standard, setStandard] = useState<Standard | null>(null)
  const [sections, setSections] = useState<Section[]>([])

  useEffect(() => {
    const stored = sessionStorage.getItem('customQuizSelectedStandard')
    if (stored) {
      const s = JSON.parse(stored) as Standard
      setStandard(s)
      setSections(s.sections || [])
    } else {
      router.push('/custom-quiz/grade')
    }
  }, [router])
  ...
  const handleSectionSelect = (section: Section) => {
    ...
    sessionStorage.setItem('customQuizGradeAndSection', JSON.stringify(payload))
    router.push('/custom-quiz/students')
  }
